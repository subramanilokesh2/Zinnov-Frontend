...
import { jsonFetch } from "../api"; // hosted API helper
...
const pan = keyframes`0%{background-position:0 0,0 0}100%{background-position:300px 180px,-300px -180px}`;

/* ---------- Theme (same palette/typography as Upload.jsx) ---------- */
...
const fromNow = (date) => {
  const d = new Date(date); const diff = Math.round((d.getTime() - Date.now())/1000);
  const map = [[60,"second"],[60,"minute"],[24,"hour"],[7,"day"],[4.345,"week"],[12,"month"],[1e9,"year"]];
  let v = Math.abs(diff), unit="second";
  for (const [step, name] of map){ if (v < step){ unit=name; break;} v/=step; }
  return rtf.format(Math.round(v) * Math.sign(diff), unit);
};
...
function KPI({ icon, label, value, hint, loading, accent="primary", bars=[], delta }) {
  const display = useCountUp(loading ? 0 : Number(String(value).replace(/\D+/g,"")) || 0);
...
export default function Dashboard() {
  // Upload.jsx uses light by default — mirror that:
  ...
  // Prevent going back/forward after login + load user from storage (+ optional auth guard)
  useEffect(() => {
    // 1) Block Back/Forward via history manipulation
    const keepHere = () => {
      try {
    ...
    // 2) Block keyboard navigation (Alt+←/→, Meta+[ / ], Backspace outside inputs)
    const keyNavBlocker = (e) => {
      const tag = (e.target?.tagName || "").toLowerCase();
      const isEditable =
        tag === "input" ||
        tag === "textarea" ||
        (e.target && (e.target.isContentEditable || e.target.closest?.('[contenteditable="true"]')));
      ...
      // Block Backspace when not editing text inputs
    ...
    // 3) Guard against unloads (forward / refresh)
    const beforeUnload = (e) => {
      // Show native dialog; stops accidental navigation away
      e.preventDefault();
      e.returnValue = "";
    };
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (mod && e.key.toLowerCase() === "k") {
        e.preventDefault();
        const el = document.getElementById("global-cmd");
        el?.focus();
      }
    };
  ...
  const onLogout = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("authUser"); // ensure user info removed
      sessionStorage.clear();
      // Hard replace so user can't navigate back into the app
    ...
    (async () => {
      try {
        ...
        if (dash.status==="fulfilled") {
          const d = dash.value || {};
          if (!loading && d.imports7d > lastImports.current) setCelebrate(true);
          lastImports.current = d.imports7d || 0;
          setKpis({
            datasets: d.datasets||0, workbooks: d.workbooks||0, rowsTotal: d.rowsTotal||0,
            imports7d: d.imports7d||0, avgCols: d.avgCols||0, queryAvgMs: d.queryAvgMs||0,
            todayQueries: d.todayQueries||0, compliant: !!d.compliant,
            trendImports14: d.trendImports14||[], trendQueries14: d.trendQueries14||[]
          });
        }
      ...
      >
        ...
        >
          <Toolbar sx={{ gap: 1.25, minHeight: 72, px: { xs: 1.25, md: 3 } }}>
            ...
            {/* Command bar (same styling as Upload) */}
        ...
        {/* Main container (match Upload’s structure) */}
                  ...
                  {!!(insights.topTags||[]).length && (
                    <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: "wrap" }}>
                      {(insights.topTags||[]).map(t=>(
                        <Chip key={t.tag} label={`#${t.tag}`} size="small" sx={{ bgcolor: theme.palette.mode==="light" ? "rgba(0,0,0,.04)" : "rgba(255,255,255,.06)" }} />
                      ))}
                    </Stack>
                  )}
                </Box>
                        ...
                        ? activity.map((a,i)=>(
                            <Paper key={i} variant="outlined" sx={{ p: 1, borderRadius: 2 }}>
                              <Stack direction="row" spacing={1} alignItems="center">
                                <Chip size="small" label={a.evt.replace(/_/g," ")} sx={{ textTransform:"capitalize" }} />
                        ...
                        ? topSheets.map((s,i)=>(
                            <Stack key={s.id ?? i} direction="row" spacing={1} alignItems="center">
                              ...
                              <Box sx={{ overflow:"hidden" }}>
                                ...
                                <Typography variant="caption" color="text.secondary">{nf.format(s.row_count)} rows</Typography>
                              </Box>
                            </Stack>
                          ))
                        ...
                        <Paper variant="outlined"
                          ...
                          <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{ fontWeight: 800, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                              {loading ? <Skeleton width={140}/> : wb.name}
                            </Typography>
                            <Typography variant="caption" color="text.secondary">
                              {loading ? <Skeleton width={100}/> : `Updated ${fromNow(wb.updated_at)}`}
                            </Typography>
                          </Box>
                          <Chip size="small" label={loading ? "…" : `${(wb.sheets||[]).length} sheets`} />
                        </Paper>
                      </Grid>
                    ))}
                  </Grid>
